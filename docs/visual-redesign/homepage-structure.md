...
- Place an oversized, stylized slash directly over that shared diagonal edge so it marks the actual boundary between the two links. Extend it beyond the section's top and bottom visual bounds and use monochrome parallel strokes or an outlined blade treatment rather than a plain rule. Implement it as decorative pseudo-elements with `aria-hidden="true"` semantics and `pointer-events: none` so it never blocks either link.
- Prevent the oversized slash from creating horizontal page overflow. Do not clip the focus outline of either linked side.
- At compact widths, stack residential before business. Give the residential panel a slanted bottom edge and the business panel the matching slanted top edge, then reduce the slash to an angled seam over that boundary. Preserve both full-panel links and their complete labels.
- Use monochromatic graphite-and-off-white architectural imagery related to the service-card render system, but make the audience renders visibly more solid and environmental. Limit translucency and exposed technical linework to small supporting regions.
...
- Keep business image production deferred. The layout may use an explicit placeholder until the business audience image brief and asset are approved.
